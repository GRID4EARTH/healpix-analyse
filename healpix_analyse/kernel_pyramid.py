...
Note that the literal ``B_K = W K S`` choice above is not merely expensive
to compute -- for the masked/hole-filling use this pyramid exists for, it is
*useless*, because ``y = S B_K W x = S W K S W x = K x`` (using ``S W = I``
twice): the multiscale structure cancels out completely and the pipeline
degenerates to a single, ordinary convolution with ``K`` at the finest
resolution alone, with only ``K``'s own compact support to reach across a
hole. A block-diagonal ``B_K`` is not a shortcut around computing the dense
operator; it is the thing that actually keeps per-band information distinct
so that filling can draw on coarser, wider-reaching bands.

**Where does each band's kernel come from?** ``from_kernel`` takes a
continuous profile ``kernel(rho_pix, phi_rad)`` and realizes it, once, on
the exact discrete stencil geometry of the *finest* band (``J=0``, see
``weights_from_finest_band`` below); every coarser band's ``HealPixConv``
reuses that identical tap vector, unchanged. The pyramid is *built from* a
single kernel defined at the finest resolution, not from ``n_bands``
independent re-evaluations of the same formula on each band's own,
slightly different real HEALPix geometry (nearest-neighbour distances on
the sphere are not perfectly self-similar across ``nside`` the way an
idealized flat grid's would be). Re-evaluating per band was the previous
default and is still available (``weights_from_finest_band=False``) for
comparison, but it means two bands nominally sharing "the same" kernel can
...
class HealPixKernelPyramid:
    ...
    def from_kernel(
        ...
        *,
        ...
        channels: int = 1,
        weights_from_finest_band: bool = True,
        ...
        kernel : KernelFn
            ``fn(rho_pix, phi_rad) -> weight``. By default (see
            ``weights_from_finest_band``) evaluated exactly once, on band 0's
            (the finest band's) own stencil geometry (see
            :func:`_stencil_pixel_polar`), and that single discrete tap
            vector is then reused unchanged, tap for tap, to build every
            band's ``HealPixConv`` -- the pyramid is *constructed from* one
            kernel given at the finest resolution, not from independent
            per-band reconstructions of it. The kernel's compact support (in
            pixel units) is therefore identical band to band by
            construction, not merely by the profile's own scale-invariance;
            its physical footprint still grows with the coarsened pixel size.
        ...
        bands : sequence of int, optional
            ...
            The finest band's geometry (``decomp.levels[0]``) is still used
            to realize the shared kernel weights even if band 0 itself is
            excluded here.
        weights_from_finest_band : bool, default True
            If ``True`` (default, see above), ``kernel`` is sampled once, on
            band 0's stencil, and every band shares that identical tap
            vector. If ``False``, restores the previous behaviour: ``kernel``
            is re-evaluated independently at each band's own
            :func:`_stencil_pixel_polar` geometry. The two differ only
            because real HEALPix pixel geometry is not perfectly
            self-similar across resolutions -- kept for comparison, not
        ...
        w_shared: Optional[np.ndarray] = None
        if weights_from_finest_band:
            nside_finest = 2 ** decomp.levels[0]
            rho_pix_0, phi_rad_0 = _stencil_pixel_polar(compact_kernel_sz, nside_finest)
            w_shared = np.asarray(kernel(rho_pix_0, phi_rad_0), dtype=np.float64)
            if w_shared.shape != rho_pix_0.shape:
                raise ValueError(
                    "kernel(rho_pix, phi_rad) must return an array the same "
                    f"shape as its inputs ({rho_pix_0.shape}); got {w_shared.shape}"
                )
